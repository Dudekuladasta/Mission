pipeline {
  ...
  stages {
    stage('Git Checkout') {
      steps {
        git branch: 'main', changelog: false, credentialsId: 'git-cred', poll: false, url: 'https://github.com/Dudekuladasta/Mission.git'
      }
    }
    stage('Compile') {
      steps {
      ...
      steps {
        script {
          withDockerRegistry(credentialsId: 'docker-cred', toolName: 'docker') {
            sh "docker build -t dastaaws/mission:latest ."
          }
        }
      }
    }
    stage('Trivy Scan Image') {
      steps {
        sh "trivy image --format table -o trivy-image-report.html dastaaws/mission:latest"
      }
    }
    stage('Publish Docker Image') {
      steps {
        script {
          withDockerRegistry(credentialsId: 'docker-cred', toolName: 'docker') {
            sh "docker push dastaaws/mission:latest"
          }
        }
      }
    }
